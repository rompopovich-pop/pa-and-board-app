...
- **No format a person has to remember.** Dates use the platform's own picker (calendar on Android, wheel on iOS) and are displayed back in the reader's long format — never an input asking for `YYYY-MM-DD`. The same principle covers anything else with a "correct" shape: the app knows the format, so the owner shouldn't have to.
- **A phone number on a record is an action, not a string.** Wherever one is shown, offer calling it and opening a WhatsApp chat with it, so reaching a client never means copying digits into another app.
...
**The most important purchase surface is not the upgrade screen.** It is the contextual upsell on a Board client record — the moment an owner reaches for "have my assistant send this to Dana" without the PA plan. `payments-spec.md` section 4a specifies when it fires and what it says; what matters here is that it is **a confirmation card like every other confirmation card** (section 2) — same rounded shape, same attention accent, same confirm/decline pair — and never an interstitial, a modal takeover or a visual language borrowed from advertising. It appears only when the owner taps the action, never on its own, and the store-mandated disclosures below apply to it in full, because it is a place where money changes hands.
